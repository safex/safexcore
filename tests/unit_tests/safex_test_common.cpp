...
tx_destination_entry create_safex_feedback_destination(const cryptonote::account_public_address  &to)
{
    return tx_destination_entry{0, to, false, tx_out_type::out_safex_feedback};
}
  ...
  BOOST_FOREACH (const block &blk, blockchain)
        {
          ...
          for (size_t i = 0; i < vtx.size(); i++)
          {
            ...
            for (size_t j = 0; j < tx.vout.size(); ++j)
            {
              ...
              {
                if (out.target.type() == typeid(cryptonote::txout_to_key) || (out.target.type() == typeid(cryptonote::txout_to_script) && out_type == cryptonote::tx_out_type::out_safex_purchase))
                { // out_to_key
                  output_index oi(out.target, out.amount, out.token_amount, boost::get<txin_gen>(*blk.miner_tx.vin.begin()).height, i, j, &blk, vtx[i]);
...
void fill_create_offer_tx_sources_and_destinations(map_hash2tx_t &txmap,  std::vector<block> &blocks, const cryptonote::account_base &from, uint64_t token_amount,
...
void fill_edit_offer_tx_sources_and_destinations(map_hash2tx_t &txmap,  std::vector<block> &blocks, const cryptonote::account_base &from, uint64_t token_amount,
                                                   uint64_t fee, size_t nmix, const crypto::public_key &pkey, const safex::safex_offer &sfx_offer, std::vector<tx_source_entry> &sources,
                                                   std::vector<tx_destination_entry> &destinations)
{
    sources.clear();
...
void fill_create_purchase_tx_sources_and_destinations(map_hash2tx_t &txmap,  std::vector<block> &blocks, const cryptonote::account_base &from, uint64_t cash_amount,
                                                  uint64_t fee, size_t nmix, const safex::safex_purchase &sfx_purchase, const cryptonote::account_public_address seller_address, std::vector<tx_source_entry> &sources,
                                                  std::vector<tx_destination_entry> &destinations)
{
    sources.clear();
    destinations.clear();
...
//        throw std::runtime_error("couldn't fill transaction sources");
    //fill cache sources for fee
    if (!fill_tx_sources(txmap, blocks, sources, from, 0, nmix, cryptonote::tx_out_type::out_safex_purchase))
...
void fill_create_feedback_tx_sources_and_destinations(map_hash2tx_t &txmap,  std::vector<block> &blocks, const cryptonote::account_base &from, uint64_t cash_amount,
                                                      uint64_t fee, size_t nmix, const safex::safex_feedback &sfx_feedback, std::vector<tx_source_entry> &sources,
                                                      std::vector<tx_destination_entry> &destinations)
{
    sources.clear();
    destinations.clear();
    ...
    if (!fill_tx_sources(txmap, blocks, sources, from, fee, nmix, cryptonote::tx_out_type::out_cash))
        throw std::runtime_error("couldn't fill transaction sources");
    //fill cache sources for feedback token
    if (!fill_tx_sources(txmap, blocks, sources, from, 0, nmix, cryptonote::tx_out_type::out_safex_feedback))
...
bool construct_create_purchase_transaction(map_hash2tx_t &txmap,  std::vector<block> &blocks, cryptonote::transaction &tx, const cryptonote::account_base &from,
                         uint64_t fee, size_t nmix, const safex::safex_purchase &sfx_purchase, const cryptonote::account_public_address seller_address)
{
    std::vector<tx_source_entry> sources;
    std::vector<tx_destination_entry> destinations;
    fill_create_purchase_tx_sources_and_destinations(txmap, blocks, from, sfx_purchase.price, fee, nmix, sfx_purchase, seller_address, sources, destinations);

    return construct_tx(from.get_keys(), sources, destinations, from.get_keys().m_account_address, std::vector<uint8_t>(), tx, 0);
}

bool construct_create_feedback_transaction(map_hash2tx_t &txmap,  std::vector<block> &blocks, cryptonote::transaction &tx, const cryptonote::account_base &from,
