...
Core tests take longer than any other Safex tests, due to the high amount of computational work involved in validating core components.
...
To run only Safex's core tests (after building):
...
To run only Safex's crypto tests (after building):
...
To run only Safex's hash tests (after building):
...
To run only Safex's performance tests (after building):
...
To run only Safex's unit tests (after building):
